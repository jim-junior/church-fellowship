...
import { getTestimonies } from "../store/schoolSheetSlices/schoolStore";
...
const Testimonies = () => {
    ...
    const { testimonies } = useSelector(state => state.fellowShipStore);
    useEffect(() => {
            ...
            {testimonies.map((tes) => {
                return (
                    <div className="py-3 border-b border-gray1">
                        ...
                        <p className="text-xs text-gray5 mt-2">{tes.testimony}</p>
                    </div>
                )
            })}
